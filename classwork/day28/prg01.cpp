...
rules:
1.when the size is equalto cap,cap is doubled by its earlier value=>resizing the array.
...
int myArray::push_back(int v) {
	if (size == capacity)
		cout << "Cap is equalto size when adding element " << v << endl;
		resize();
	arr[size++] = v;
	return 0;
}
...
int myArray::resize() {
	...
	//copy the content of earlier data to new data
	for (int i = 0;i < size;i++)
		newData[i] = arr[i];
	delete[] arr;
